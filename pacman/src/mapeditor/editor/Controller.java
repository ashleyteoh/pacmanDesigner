...
public class Controller implements ActionListener, GUIInformation {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("flipGrid")) {
			...
			String path = "pacman/properties/test.properties";
			SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
				@Override
				protected Void doInBackground() throws Exception {
					Driver.runGameDriver(path, currFile);
					return null;
				}
			};
			worker.execute();
	...
	private void saveFile() {
		...
		try {
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				CheckerAdapter checker = new LevelCheckerAdapter(model, chooser.getSelectedFile());
				checker.checkFile();
			}
		...
		try {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (selectedFile.canRead() && selectedFile.exists()) {
					loadTiles(selectedFile);
					String mapString = model.getMapAsString();
					grid.redrawGrid();
					CheckerAdapter checker = new LevelCheckerAdapter(model, selectedFile);
					checker.checkFile();
				}
			}
		} catch (Exception e) {
...
//			selectedFile = new File(file);
	...
	public void loadTiles(File selectedFile) throws IOException, JDOMException {
